 ...
 * Version: 1.2
 *
 * $Id: osal.h 6680 2018-03-30 02:02:40Z songkx $
 *--------------------------------------------------------------------------*/
#pragma once
#ifndef _OSAL_H_
#define _OSAL_H_
...
#define ERR_NOT_RECORD			(-1027) /* No record*/

#define ERR_LIB_NOT_FOUND		(-1028)	/* Library does not exist */

#define RET_RECORDING			1
...
#define ERR_OS_VERSION_TOO_LOW	(-2209) /* OS Version is too low */
...
#define SM2_DATA_MAX_LEN    2048    /* SM2 Sign/Verify max length */
...
#define REAL_PRN_WIDTH_3INCH		576
#define VIRTUAL_PRN_WIDTH_3INCH		576
...
#define TYPE_WIRELESS_VER	19  /* Wireless version*/
...
#define ERR_WL_REATTACH_GPRS	(-3520)	/* Reattach GPRS error */

#define WL_CSD_READY		3
#define WL_GPRS_CSD_READY		4
...
#define BATTERY_LEVEL_ABNORMAL	8	/* Battery is abnormal */
...
#define USB_ERR_RECV_BUF	(-3418) /* recvBuf is is not enough*/


/*--------------------------------------------------------------------------
 * barcode 
 *--------------------------------------------------------------------------*/
#define ERR_BAR_CAMER_INIT_FAILED               (-3601) /* camera init failed */
#define ERR_BAR_ZSDECODE_INIT_FAILED            (-3602) /* zsdcode init failed */
#define ERR_BAR_SRECODE_INIT_FAILED             (-3603) /* srcode init failed  */
#define ERR_BAR_NOT_ENOUGH_BUFFER               (-3604) /* out data buffer not enough, you should set more large outdata buffer */
#define ERR_BAR_READ_CAMERA_BUFFE_FAILED        (-3605) /* read camera buffer data failed  */
#define ERR_BAR_DECODE_FAILED                   (-3606) /* decode failed */

/*--------------------------------------------------------------------------
 * camera
 *--------------------------------------------------------------------------*/
#define RESOLUTION_WIDTH_640_HEIGHT_480			(1 << 0)

#define CAMERA_PIXEL_FORMAT_YUYV				(1 << 0)
#define CAMERA_PIXEL_FORMAT_RGB565				(1 << 1)

/*--------------------------------------------------------------------------
 *GPS
 * ------------------------------------------------------------------------*/
 #define ERR_GPS_POWER_ONING   (-3901)
...
#define NET_LINK_BT			9	/* bluetooth PAN */
...
#define PORT_USBHID			13	/* USB Scanner */
#define PORT_USBHOST1		14	/* USB port host */
...
/* barcode */
#define USE_BACK_CAMERA			1
#define USE_FRONT_CAMERA		2
#define OPEN_BACK_ADDBRILAMP            10
#define CLOSE_BACK_ADDBRILAMP           11
#define OPEN_BACK_POSITIONLAMP          12
#define CLOSE_BACK_POSITIONLAMP         13
#define OPEN_FRONT_ADDBRILAMP           20
#define CLOSE_FRONT_ADDBRILAMP          21
#define OPEN_FRONT_POSITIONLAMP         22
#define CLOSE_FRONT_POSITIONLAMP        23
...
typedef enum {
	BASE_STATE_OPENED = 0,
	BASE_STATE_CONNECTING,
	BASE_STATE_CONNECTED,		/* connected, but not opened */
} BASE_STATE;
...
int OsBaseScan(ST_HandsetLinkInfo *links, int n, int timeout);
int OsBaseConnect(ST_HandsetLinkSet set, int timeout);
int OsBaseDisconnect(void);
 ...
 * Record
 *--------------------------------------------------------------------------*/
int OsRecordOpen(void);
int OsRecordStart(const char *FileName, const char *Type, unsigned int Channel, const char *Format, unsigned int Rate, unsigned int Duration);
int OsRecordStop(unsigned int *Time);
int OsRecordCheck(void);
void OsRecordClose(void);

/*--------------------------------------------------------------------------
 * Wavplay
 *--------------------------------------------------------------------------*/
int OsPlayWave(const char *Buf, int Len, int Volume, int DurationMs);
int OsStopPlayWave(void);
...
int OsWlReAttachGprs(char* CmeString, int Size);
...
int OsPedGetPinLength(int KeyIdx, const uchar *DataIn, int Mode, uchar *PinBlock);		
int OsPedSetPinBg(uchar Mode,const uchar * Bg,int BgLen);
int OsPedCustomKeypad(char *IconPath, uchar *KeypadColorBg, uchar Mode);
int OsPedSetOfflinePin(uchar Mode,uchar TpkIdx,uchar * PinBlock,ushort PinBlockLen);
...
int OsScanSetParam(unsigned int Param);
 ...
 * Camera 
 *--------------------------------------------------------------------------*/
int OsCameraOpen(int Index);
void OsCameraClose(void);
int OsCameraSetParam(CameraAttribute* CameraParam);
int OsCameraGetParam(CameraAttribute* CameraParam);
int OsCameraCapture(unsigned char* Buf, unsigned int Size);
